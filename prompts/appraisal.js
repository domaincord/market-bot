...
exports.run = async (msg, domains) => {
    ...
    const appraisals = await Promise.all(domains.map(getAppraisals))
    ...
    if (notListed.length) {
        ...
        collector.on('collect', async reaction => {
            ...
            await bootstrapListing(reaction.message.embeds[0].title, msg.member.user, reaction.message)
        });
...
const getAppraisals = async (domain) => {
    ...
    const appraisalData = snapshot.docs.map(doc => {
        const appraisalsSnapshot = doc.collection('appraisals').get()
        const appraisals = appraisalsSnapshot.size > 0 ? appraisalsSnapshot.docs.map(apDoc => {
           return {
                _id: apDoc.id,
                ...apDoc.data()
           }
...
const appraise = async (domain, value) => {
